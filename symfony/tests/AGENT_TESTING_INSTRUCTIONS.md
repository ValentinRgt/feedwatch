...
#### HTML scraping (FW-10)
- `HTMLService::read()` now parses items via `DomCrawler` + XPath using the selectors stored on `Source` (`itemContainer`, `itemTitle`, `itemLink`, optional `itemPublishedAt`). It throws `InvalidArgumentException` when `hasRequiredItemSelectors()` is false. Unit coverage in `tests/Unit/Service/FeedReader/HTMLServiceCest.php` exercises: format support, unchanged-checksum short-circuit, missing-selectors exception, fresh-content parsing + checksum, ArticleDTO mapping, and the optional `itemPublishedAt` branch.
- The HTML form path (selectors required when `format=html`, skipped for XML, `itemPublishedAt` optional) is covered through `tests/Functional/Controller/SourceControllerCest.php` (per §2: forms are tested indirectly via the controller). The `SourceType` `PRE_SUBMIT` listener must read the format from `$event->getData()` — not from `$form->get('format')->getData()`, which returns the *initial* value during `PRE_SUBMIT` and would skip validation on creation.
